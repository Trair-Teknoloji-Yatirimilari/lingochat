...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // If user not in DB, check if it's a phone auth user (openId starts with "phone_")
    if (!user && sessionUserId.startsWith("phone_")) {
      console.log("[Auth] Phone auth user not found in DB, skipping OAuth sync");
      throw ForbiddenError("User not found");
    }

    // If user not in DB and not phone auth, sync from OAuth server automatically
    if (!user) {
      try {
